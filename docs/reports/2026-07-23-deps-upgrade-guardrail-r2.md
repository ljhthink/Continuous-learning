...
| --- | --- | --- | --- | --- | --- |
| kb_get_page | [read-only.ts:198](file:///D:/s0611/code/Continuous-learning/server/src/tools/read-only.ts#L198) | YAMLException 未捕获 → 工具崩溃（500） | 降级空 frontmatter → use_count seeding 为 1 → 写回 `{use_count:1}`+原始 body | 无（malformed frontmatter 本就无效，丢弃合理；读取操作尽力返回 body 正确） | 正确 |
| kb_promote_experience | [write.ts:230](file:///D:/s0611/code/Continuous-learning/server/src/tools/write.ts#L230) | YAMLException 未捕获 → 工具崩溃 | 降级空 frontmatter → [write.ts:237](file:///D:/s0611/code/Continuous-learning/server/src/tools/write.ts#L237) 状态机检查 `type !== "experience"` fail-fast 返回错误 | 无（状态机在写操作前 fail-fast，malformed frontmatter 不会被覆盖） | 正确 |
| /dream | [dream.ts:109](file:///D:/s0611/code/Continuous-learning/server/src/dream.ts#L109) | YAMLException 未捕获 → 中断整个批处理，后续卡片全部跳过 | 降级空 frontmatter → [dream.ts:110](file:///D:/s0611/code/Continuous-learning/server/src/dream.ts#L110) `type !== "experience"` continue 跳过该页，批处理继续 | 无（malformed 页面被安全跳过，不触发 demote 写操作） | 正确（批处理健壮性显著改善） |
| kb_lint | [lint.ts:198](file:///D:/s0611/code/Continuous-learning/server/src/tools/lint.ts#L198) | loadAllPages 的 try/catch 捕获 → stderr 记录 → 跳过该页（不在 pages 列表）→ checkFrontmatter 看不到 → 不报告 | 降级空 frontmatter → 页面加入 pages 列表 → checkFrontmatter 报告 "Missing required fields"（high） | 无（lint 只读分析） | 改善（malformed 页面从静默跳过变为显式 high 报告） |
| kb_list_categories | [read-only.ts:116](file:///D:/s0611/code/Continuous-learning/server/src/tools/read-only.ts#L116) | 已有 try/catch 包裹 | 行为不变（原本就降级） | 无 | 无影响 |
| kb_search | [search.ts:67](file:///D:/s0611/code/Continuous-learning/server/src/tools/search.ts#L67) | 需确认是否有保护 | 修复后同样受益于降级 | 无 | 改善 |
...
- `loadAllPages` 的 try/catch（[lint.ts:196-237](file:///D:/s0611/code/Continuous-learning/server/src/tools/lint.ts#L196-L237)，已有）：捕获 `readFile` I/O 错误（文件权限、磁盘错误），跳过不可读页面并 stderr 记录。
...
| tsconfig 加载策略 | [tsconfig.json:6](file:///D:/s0611/code/Continuous-learning/server/tsconfig.json#L6) `types: ["node"]` | 仅加载 @types/node，@types/* 不自动加载 |
...
- [line 1145-1147](file:///D:/s0611/code/Continuous-learning/server/node_modules/js-yaml/dist/js-yaml.mjs#L1145-L1147)：`DEFAULT_CONSTRUCTOR_OPTIONS = { ..., schema: CORE_SCHEMA, ... }`
- [line 2238-2242](file:///D:/s0611/code/Continuous-learning/server/node_modules/js-yaml/dist/js-yaml.mjs#L2238-L2242)：`load(input, options)` 调用 `loadDocuments(input, options)`，不传 schema 时使用默认 `CORE_SCHEMA`
- [line 708](file:///D:/s0611/code/Continuous-learning/server/node_modules/js-yaml/dist/js-yaml.mjs#L708)：`CORE_SCHEMA` 是标准 YAML 核心 schema
...
| 路径遍历 | 不在本轮范围（调用方已有 traversal 检查，如 [read-only.ts:188-191](file:///D:/s0611/code/Continuous-learning/server/src/tools/read-only.ts#L188-L191)） |
...
**位置**：[frontmatter.ts:29-31](file:///D:/s0611/code/Continuous-learning/server/src/utils/frontmatter.ts#L29-L31)
...
**建议**（可选，非强制）：在 catch 块添加 `console.error` debug 日志，与项目其他路径（如 [read-only.ts:214](file:///D:/s0611/code/Continuous-learning/server/src/tools/read-only.ts#L214)、[lint.ts:236](file:///D:/s0611/code/Continuous-learning/server/src/tools/lint.ts#L236)）的"不吞异常"模式一致（CLAUDE.md §19.4）。示例方向：
...
**位置**：[read-only.test.ts:250-267](file:///D:/s0611/code/Continuous-learning/server/src/tests/read-only.test.ts#L250-L267)
